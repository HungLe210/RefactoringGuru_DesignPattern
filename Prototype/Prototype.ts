...
// Concrete prototype with circular reference.
class Circle extends Shape {
    public radius: number;
    public relatedCircle: Circle | null = null;

    constructor(radius: number = 0, relatedCircle: Circle | null = null, X: number = 0, Y: number = 0, color: string = 'white') {
        super(X, Y, color);
        this.radius = radius;
        this.relatedCircle = relatedCircle ? new Circle(relatedCircle.radius, null, relatedCircle.X, relatedCircle.Y, relatedCircle.color) : null;
    ...
    clone(): Shape {
        return new Circle(this.radius, this.relatedCircle, this.X, this.Y, this.color);
    }
}
...
function clientCode() {
    ...
    const test = Object.create(circle1, { name: { value: true, enumerable: true } });
